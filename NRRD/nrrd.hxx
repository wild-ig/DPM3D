		...
		std::map<std::string,std::string>* hdr_fields=0x0)
	{
		...
		else
		{
			...
			*data=new T[n];
			...
			// Image data located in another file
			std::string data_file=file;
			{
				auto it=hdr_fields->find("datafile");
				if (it==hdr_fields->end()) it=hdr_fields->find("data file");
				if (it!=hdr_fields->end())
				{
					data_file=it->second;
					std::cerr << "NDDR::load<T>(...): External data file " << data_file << " .\n";
					hdr_fields->erase(it);
					header_offset=0;
				}
			}

			// Number of bytes to skip at beginning of data block
			{
				auto it=hdr_fields->find("byteskip");
				if (it==hdr_fields->end()) it=hdr_fields->find("byte skip");
				if (it!=hdr_fields->end())
				{
					int byte_skip=stringTo<int>(it->second);
					// Negative byte skip not supported
					if (byte_skip<0)
					{
						std::cerr << "NDDR::load<T>(...): negative \"byte skip\" not supported.\n";
						return false;
					}
					header_offset+=byte_skip;
					hdr_fields->erase(it);
				}
			}

			// Number of lines to skip: not supported
			if (hdr_fields->find("line skip")!=hdr_fields->end()||hdr_fields->find("lineskip")!=hdr_fields->end())
			{
				std::cerr << "NDDR::load<T>(...): \"line skip\" not supported.\n";
				return false;
			}
